#ifndef LEGACY_RNNLM_READER_H__
#define LEGACY_RNNLM_READER_H__
#include "rnnlmlib.h"
#include <string>
using std::string;
 ...
 public:
  ...
  Decodable CopyLegacyRnnLM (Hasher& h, int max_order = 8) {
    ...
    for (int i = 0; i < rnnlm_.layer0_size * rnnlm_.layer1_size; i++) 
      d.syn0.push_back ((double)rnnlm_.syn0 [i].weight);

    for (int i = 0; i < rnnlm_.layer1_size * rnnlm_.layer2_size; i++) 
      d.syn1.push_back ((double)rnnlm_.syn1 [i].weight);
    
    for (int i = 0; i < rnnlm_.direct_size; i++)
      d.synd.push_back ((double)rnnlm_.syn_d [i]);
